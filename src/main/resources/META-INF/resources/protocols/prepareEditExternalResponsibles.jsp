...
<p class="breadcumbs">
	...
	<span><strong><bean:message key="label.protocol.edit.step3" bundle="PROTOCOLS_RESOURCES"/></strong></span> >
...
function removeUnitFunction(formId) {
	if(confirm("Tem a certeza que pretende remover a unidade?")) {
		document.forms[formId].innerHTML = document.forms[formId].innerHTML + "<input type=\"hidden\" name=\"removeExternalUnit\" value='removeExternalUnit'>";
...
function removePositionFunction(formId, position) {
	if(confirm("Tem a certeza que pretende remover o cargo?")) {
		document.forms[formId].innerHTML = document.forms[formId].innerHTML + "<input type=\"hidden\" name=\"removePositionExternal\" value='removePositionExternal'>"+
...
function removePersonFunction(formId, personId) {
	if(confirm("Tem a certeza que pretende remover a pessoa seleccionada?")) {
		document.forms[formId].innerHTML = document.forms[formId].innerHTML + "<input type=\"hidden\" name=\"removePersonInExternalUnit\" value='removePersonInExternalUnit'>" +
...
<fr:form action="/protocols.do?method=updateEditionBean" id="<%= unitOID.toString() %>">
...
<html:submit bundle="PROPERTIES_RESOURCES" property="insertExternalPosition">
...
<fr:form action="/protocols.do?method=updateEditionBean">
